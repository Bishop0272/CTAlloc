...
def ct_alloc():
    ...
    if os.path.isfile(credentialsPath):
        ...
        # Define the email list with weighted probabilities
        emails = [
        ...
        # Assign weights to each email, with higher weights for Manjeshkar and Menaga
        email_weights = [
            2,  # menaga.sounderaj@designcafe.com
            2,  # aruna.nagaraj@designcafe.com
            2,  # harishkumar.r@designcafe.com
            2,  # manjeshkar.reddy@designcafe.com
        ]

        # Function to generate a random email with weighted probabilities
        def generate_ct_email():
            return random.choices(emails, weights=email_weights, k=1)[0]
        ...
        for i, row in enumerate(data):
            ...
            if ((is_blank(payment_15_date)) and not(is_blank(kickoff_date))) and not is_blank(sign_up_date) and payment_15_date != sign_up_date and is_blank(email_15):
                ct_email = generate_ct_email()
                updates.append({'range': f'O{row_number}', 'values': [[ct_email]]})  # Update 'CT-Email (15%)'
            ...
            if (not is_blank(siri_date) and is_blank(payment_35_date)) and not is_blank(payment_15_date) and not is_blank(sign_up_date) and payment_35_date == payment_15_date and is_blank(email_35):
                ct_email = generate_ct_email()
                updates.append({'range': f'P{row_number}', 'values': [[ct_email]]})  # Update 'CT-Email (35%)'
